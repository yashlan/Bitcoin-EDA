...
# Create lagged features for Gradient Boosting
...
def forecast_and_evaluate(ax, forecast_days, color):
    ...
    y_true_gb = data['Close'].iloc[-forecast_days:].values
    mse_gb = mean_squared_error(y_true_gb, forecast)
    mae_gb = mean_absolute_error(y_true_gb, forecast)
    
    # Calculate MAPE and Accuracy
    mape_gb = np.mean(np.abs((y_true_gb - forecast) / y_true_gb)) * 100
    accuracy_gb = 100 - mape_gb
    
    print(f'Gradient Boosting ({forecast_days} days) - MSE: {mse_gb:.2f}, MAE: {mae_gb:.2f}, MAPE: {mape_gb:.2f}%, Akurasi: {accuracy_gb:.2f}%')
...
# Adjust layout
fig.suptitle('Prediksi Harga Bitcoin dengan Model Gradient Boosting', fontsize=16)
